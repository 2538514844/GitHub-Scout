...
- **NVENC encode args are defined once in `electron/ipc.js` as `NVENC_ENCODE_ARGS`** and shared by both `transcodeToMp4` (main.js) and `handleTestRender` (ipc.js) via spread. Current params: `h264_nvenc`, `-preset p4`, `-rc constqp`, `-qp 18`, `-g 60`, `-vf scale=3840:2160...`, `-c:a aac -b:a 320k -movflags +faststart`. **No `-level:v` parameter**: CQP (Constant QP) mode does not enforce VBV bitrate constraints, so a level parameter is unnecessary — and some NVIDIA driver versions explicitly REJECT any level value in CQP mode with "InitializeEncoder failed: invalid param (8): Invalid Level". **No `-maxrate`/`-bufsize`**: same reason. When modifying encode args, update `NVENC_ENCODE_ARGS` (and corresponding `CPU_ENCODE_ARGS`) in ipc.js — both test and production paths reference the same constants.
